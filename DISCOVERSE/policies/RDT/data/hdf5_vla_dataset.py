...
class HDF5VLADataset:
    ...
    def parse_hdf5_file(self, file_path):
        ...
        with h5py.File(file_path, 'r') as f:
            ...
            qpos = qpos / np.ones((1, qpos.shape[1]))
            target_qpos = f['action'][step_id:step_id+self.CHUNK_SIZE] / np.ones((1, qpos.shape[1]))
            ...
            cam_high = parse_img('0')
            # For step_id = first_idx - 1, the valid_len should be one
            valid_len = min(step_id - (first_idx - 1) + 1, self.IMG_HISORY_SIZE)
            cam_high_mask = np.array(
                [False] * (self.IMG_HISORY_SIZE - valid_len) + [True] * valid_len
            )
            cam_left_wrist = parse_img('1') # fake img
            cam_left_wrist_mask = np.array([False] * self.IMG_HISORY_SIZE) # mask fake left img
            cam_right_wrist = parse_img('1')
    ...
    def parse_hdf5_file_state_only(self, file_path):
        ...
        with h5py.File(file_path, 'r') as f:
            ...
            # Rescale gripper to [0, 1]
            qpos = qpos / np.ones((1, qpos.shape[1]))
            target_qpos = f['action'][:] / np.ones((1, qpos.shape[1]))
